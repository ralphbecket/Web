...
var loadToDos = function () {
    return JSON.parse(localStorage.getItem("todos") || "[]").map(function (x) { return ({
        id: x.id, what: x.what, done: Obs.of(x.done)
    }); });
};
var toDos = Obs.of(loadToDos());
var saveToDos = function () {
    localStorage.setItem("todos", JSON.stringify(toDos().map(function (x) { return ({
        id: x.id, what: x.what, done: x.done()
    }); })));
};
var numTotal = Obs.fn(function () { return toDos().length; });
var numNotDone = Obs.fn(function () { return toDos().filter(function (x) { return !x.done(); }).length; });
var haveSomeToDos = Obs.fn(function () { return numTotal() !== 0; });
var haveSomeCompletedToDos = Obs.fn(function () { return toDos().some(function (x) { return x.done(); }); });
var allDone = Obs.fn(function () { return numNotDone() === 0; });
var toDoBeingEdited = Obs.of(null);
var uneditedToDoWhat = "";
var newToDo = Obs.of("");
var hiddenDoneValue = Obs.of(// All the routing one needs!
location.hash === "#active" ? true :
    location.hash === "#completed" ? false :
        null);
var numToDosShowing = Obs.fn(function () {
    return toDos().filter(function (x) { return x.done() !== hiddenDoneValue(); }).length;
});
var keyHandler = function (keys) {
    return function (v) {
        var key = v.key;
        var handler = keys && keys[key];
        if (handler)
            handler();
    };
};
var newToDoKeyHandler = keyHandler({
    "Enter": function () {
        var what = newToDo().trim();
        if (!what)
            return;
...
var editToDoKeyHandler = keyHandler({
    "Enter": function () {
...
var hideUnless = function (x) { return ({ display: x ? "" : "none" }); };
var toDoComponent = function (toDo) { return Od.component(toDo.id, function () {
    return Od.LI({
        className: (toDoBeingEdited() === toDo ? "editing " : "") +
            (toDo.done() ? "completed " : ""),
        ...
        Od.DIV({ className: "view" }, [
            ...
            Od.LABEL({
                ondblclick: function (v) {
                    ...
                    var focusEditInput = function () {
                        v.target.parentElement.nextElementSibling.focus();
                    };
            ...
            Od.BUTTON({
                className: "destroy",
                onclick: function (v) {
                    toDos(toDos().filter(function (x) { return x !== toDo; }));
                    saveToDos();
                }
            })
        ]),
...
var view = Od.component("ToDoMvcOd", function () { return Od.DIV({ id: "todoapp" }, [
    Od.HEADER({ id: "header" }, [
        Od.H1("todos"),
        Od.INPUT({
            id: "new-todo",
    ...
    Od.FOOTER({ id: "footer", style: hideUnless(haveSomeToDos()) }, [
        ...
        Od.UL({ id: "filters" }, [
            Od.LI(Od.A({
                ...
                onclick: function () { hiddenDoneValue(null); }
            }, "All")),
            Od.LI(Od.A({
                href: "#active",
                className: hiddenDoneValue() === true ? "selected" : "",
                onclick: function () { hiddenDoneValue(true); }
            }, "Active")),
            Od.LI(Od.A({
                href: "#completed",
                className: hiddenDoneValue() === false ? "selected" : "",
                onclick: function () { hiddenDoneValue(false); }
            }, "Completed"))
        ]),
        Od.BUTTON({
            id: "clear-completed",
