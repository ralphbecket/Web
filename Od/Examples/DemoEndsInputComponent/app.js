...
window.onload = function () {
    ...
    var vdom = Od.DIV([
        Od.inputComponent({
            obs: x,
            formatObs: function (n) { return n.toString(); },
            parseText: function (s) { return isFinite(s) ? +s : s; },
            updateOn: "oninput",
            props: { style: { width: "4em" } }
        }),
        "'", Od.component(null, function () { return x(); }), "'",
        Od.BUTTON({
            onclick: function () { if (typeof (x()) === "number")
                x(-x()); }
        }, "Negate!")
    ]);
