...
interface String {
    lpad(padding: string, toLength: number): string;
}
...
var ENV = ENV || (function () {
    ...
    slider.addEventListener('change', function (e) {
        ENV.mutations((e.target as any).value / 100);
        (document.querySelector('#ratioval') as any).innerHTML = 'mutations : ' + (ENV.mutations() * 100).toFixed(0) + '%';
    });
