...
(function (Test) {
    ...
    // Application state.
    var currPageName = Obs.of("Page A");
    ...
    // Add some navigation.
    var navView = function (names) {
        return Od.DIV([
            names.map(function (name) {
                return Od.P({
                    onclick: function () { log(); currPageName(name); }
                }, "Go to " + name);
            }),
            Od.HR()
        ]);
    ...
    var loggingDiv = function (name, children) { return Od.component(name, function () {
        return Od.DIV({ onodevent: function (what, dom) { log(name + ": " + what); } }, [name, children || []]);
    }); };
    var mainView = Od.component("main", function () {
